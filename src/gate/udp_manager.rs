...
use crate::{config, linux::{self, socket::Socket}, log::log_trait::Log};

use super::Gate;

impl Gate {
    pub fn open_udp_port(&self) {
        let ret = linux::socket::random_port_open();
    }

    pub fn new_udp_2_vps_line(&mut self) -> u64 {
        let id = crate::global::next_id();
        let socket = Socket::new_udp_socket();
        //socket.send();
        id
    }
...
pub fn new_udp_mainland_line(&mut self,peer_ip_port:String) {
        let id = crate::global::next_id();
        let socket = linux::socket::Socket::new_udp_socket();
    }
